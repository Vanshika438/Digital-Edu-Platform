...
const Playlists = () => {
  ...
  const toggleLessons = (playlistId) => {
  ...
  const deletePlaylist = async (playlistId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.delete(`http://localhost:5000/api/playlists/${playlistId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  
      console.log("✅ Playlist deleted:", response.data); // Debugging
  
      if (response.data.playlists) {
        setPlaylists(response.data.playlists); // ✅ Use the updated playlist list
      } else {
        console.warn("⚠️ No updated playlists received. Fetching again...");
        fetchPlaylists(); // ✅ Fetch playlists again if the response is missing data
      }
    } catch (error) {
          ...
          {playlists.length === 0 ? (
            ...
            playlists.map((playlist) => (
              <li key={playlist._id} className="playlist-item">
                ...
                <button
                  onClick={() => toggleLessons(playlist._id)}
                  className="toggle-lessons-button"
                >
                  {expandedPlaylists[playlist._id] ? "▼ Hide Lessons" : "▶ Show Lessons"}
                </button>

                <ul
                  className={`playlist-lessons ${expandedPlaylists[playlist._id] ? "expanded" : ""
                    }`}
                >
                ...
                <button
                  ...
                  disabled={deleting === playlist._id}
